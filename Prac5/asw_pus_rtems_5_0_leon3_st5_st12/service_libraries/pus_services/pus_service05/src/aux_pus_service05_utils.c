...
void pus_service5_set_EvID_enable_flag(uint16_t EvID, bool_t enable_value) {
	...
	if (index < 4) {
		uint8_t aux_EvID_offset = (EvID & 0xFFF); //Primer número 1,2,3,4 tipo de evento el resto hasta 32 para bitmap
		...
		if ((aux_EvID_offset > 0)
			...
			uint8_t mask = (0x01 << (aux_EvID_offset - 1)); //El evento uno está en la posición 0.
			if (enable_value) {
